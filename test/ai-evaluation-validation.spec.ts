import { ValidationFindingCategory, ValidationFindingSeverity } from '../src/modules/validation/domain/validation-finding.types';
import { AiEvaluationDataset } from '../evaluation/ai-validation/src/evaluation.types';
import { validateDataset, validatePredictions } from '../evaluation/ai-validation/src/evaluation.validator';
...
describe('AI evaluation validators', () => {
  ...
  it.each([
    ['duplicate case IDs', (value: AiEvaluationDataset) => value.cases.push(structuredClone(value.cases[0]))],
    ['wrong case count', (value: AiEvaluationDataset) => (value.caseCount = 2)],
    [
      'duplicate file paths',
      (value: AiEvaluationDataset) => value.cases[0].sourceFiles.push({ path: './PROGRAM.cbl', content: 'x' }),
    ],
    ['expected finding on a clean case', (value: AiEvaluationDataset) => (value.cases[0].isClean = true)],
  ...
  it.each([
    ...
    [
      'duplicate results',
      { cases: [{ caseId: 'case-1', status: 'SUCCESS', findings: [] }, { caseId: 'case-1', status: 'SUCCESS', findings: [] }] },
    ],
    ['unknown case', { cases: [{ caseId: 'missing', status: 'SUCCESS', findings: [] }] }],
    ['failure represented as empty success', { cases: [{ caseId: 'case-1', status: 'PROVIDER_FAILED', findings: [] }] }],
    ...
    expect(() => validatePredictions(value, validateDataset(dataset()))).toThrow('Invalid predictions');
  });
});
